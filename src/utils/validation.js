...
export const validateUrl = (url, required = false) => {
  ...
  try {
    new URL(url);
    // Additional checks for common protocols
 ...
 * @param {string} tier - Deck tier (FREE/PREMIUM)
 * @returns {Object} Validation result
 */
export const validatePrice = (price, tier) => {
  ...
  if (tier === DECK_TIERS.FREE) {
    ...
    if (price === null || price === undefined) {
      ...
      errors.push('Premium deck price must be greater than 0');
    } else if (price > 100) {
      errors.push('Maximum price is $100');
    }
  }
...
export const sanitizeSearchQuery = (query, maxLength = 100) => {
  ...
  if (typeof query !== 'string') {
    ...
    return { query: '', isValid: false, errors };
  }
